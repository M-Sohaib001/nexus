...
export function StudentFypDiscoveryGrid({
  initialFyps,
  currentPage = 1,
  hasNextPage = false
}: {
  ...
  const handleFilterChange = (key: string, value: string | null) => {
    ...
    router.push(`/student/fyps?${params.toString()}`, { scroll: false })
  }

  const handlePageChange = (newPage: number) => {
    const params = new URLSearchParams(searchParams.toString())
    params.set('page', String(newPage))
    router.push(`/student/fyps?${params.toString()}`)
              ...
              return (
                <Card key={fyp.id} className="flex flex-col rounded-none border-primary/10 hover:border-primary/50 transition-all duration-300 bg-background shadow-none h-full overflow-hidden relative group/card">
                  ...
                  <CardFooter className="bg-primary/5 border-t border-primary/10 p-4 flex gap-2">
                    <Link href={`/student/${student.qr_token}`} className="flex-1">
                      <Button variant="outline" className="w-full border-primary/20 text-primary hover:text-primary font-black text-[10px] tracking-widest uppercase rounded-none">ACCESS_PORTFOLIO</Button>
                    </Link>
                  </CardFooter>
                </Card>
              )
